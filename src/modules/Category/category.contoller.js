...
//  ======================= Controller function to get all subcategories for a specific category=============//
export const getSubCategoriesForCategory = async (req, res, next) => {
    try {
        const { categoryId } = req.params;

        // Find all subcategories with the specified category ID
...
export const getCategoryById = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ success: true, message: 'Category found', data: category });
    } catch (error) {
        next(error);
    }
};